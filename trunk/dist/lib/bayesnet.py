...
class RawCPT(delegate.Delegate):
    ...
    def Marginalise(self, varnames):
        ...
        temp = self.cpt
        var = list(varnames)
        var.sort(reverse=True)  # sort and reverse list to avoid inexistent dimensions
        for v in var:
            temp = na.sum(temp, axis = self.p[v])
        return temp
...
class BVertex(graph.Vertex, CPT):
    def __init__(self, name, nvalues = 2):
        '''
    ...
    def InitCPT(self):
        CPT.__init__(self, self.nvalues, self.in_v)
...
class Cluster(graph.Vertex, JoinTreePotential):
        ...
        def MessagePass(self, c):
            ...
            #print 'Message Pass from', self,'to',c 
...
class MoralGraph(graph.Graph):
        def ChooseVertex(g):
            ...
            for v,i in zip(vertices,range(len(vertices))):
                ...
                for c in cluster:
                    clusterweight[i] *= g.v[c].nvalues
                    
                for v1 in cluster:
                    clusterleft.pop(0)
                    for v2 in clusterleft:
                        if not v1 in [a.name for a in g.v[v2].adjacent_v]:
                            edgestoadd[i] += 1
        ...
        def Triangulate(G):  
            ...
            while len(G2.v):
                ...
                c = Cluster(*cluster)
                if c.NotSetSepOf(clusters): clusters.append(c)
...
class BNet(graph.Graph):
    ...
    def Moralize(self):
        ...
        for v in self.v.values():
            ...
            parentsleft = copy.copy(parents)
            
            for p1 in parents:
                parentsleft.pop(0)
                for p2 in parentsleft:
...
class JoinTree(graph.Graph):
        ...
        def __init__(self,name, BNet):
                ...
                self.likelihoods = [Likelihood(v) for v in self.BNet.v.values()]
                self.likedict = dict((v.name, l) for v,l in
                                     zip(self.BNet.v.values(), self.likelihoods))
        ...
        def GlobalPropagation(self, start = None):
            ...
            #print 'Global Propagation, starting at :', str(start)
            #print 'Collect Evidence'
            self.UnmarkAllClusters()
            start.CollectEvidence()

            #print 'Distribute Evidence'
            self.UnmarkAllClusters()
    ...
    def Marginalise(self, v, N):
        ...
        for i in range(N):
            if i > self.cut:
                ...
                vDist.cpt[state[v]] += 1
            for vname in nonEvidence:
                state[vname] = self.sampleGivenMB(vname, state)
        # CHECK: how do i normalize?
        vDist.normalise()
        return vDist
    
    def sampleGivenMB(self, vname, state):
        #NEED TO DEFINE
        return vname
