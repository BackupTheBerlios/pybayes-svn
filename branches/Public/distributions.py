...
class Gaussian_Distribution(Distribution):
    ...
    def setParameters(self, mu=None, sigma=None, wi=None, sigma_type='full', \
        ...
        try:
            self.mean = na.array(mu, shape=[self.nvalues] + \
                                 self.discrete_parents_shape, type='Float32')
        except:
        ...
        try:
            self.weights = na.array(wi, shape=[self.nvalues] + \
                                    self.parents_shape, type='Float32')
        except:
...
class MultinomialTestCase(unittest.TestCase):
    ...
    def testDictSet(self):
        ...
        print na.all(self.a.distribution[0, 0, 0, :] == na.array([-1, -1]))
        print na.all(self.a.distribution[1, 0, 0, :] == na.array([100, 100]))
        print na.all(self.a.distribution[1, 1, 0, :] == na.array([-2, -3]))        
    ...
    def testNumSet(self):
        ...
        assert(na.all(self.a.distribution[0, 0, 0, :] == \
               na.array([-1, -1])) and \
               na.all(self.a.distribution[1, 0, 0, :] == \
               na.array([100, 100])) and \
               na.all(self.a.distribution[1, 1, 0, :] == \
               na.array([-2, -3]))), \
